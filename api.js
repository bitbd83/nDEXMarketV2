...
API.init = function init(callback, allContracts, path, provider, configName, lookbackIn) {
  ...
  this.web3.version.getNetwork((error, version) => {
    ...
    // path
    if (path) {
      this.config.contractDEx = path + this.config.contractDEx;
      this.config.contractToken = path + this.config.contractToken;
    }

    // contracts
    this.contractDEx = undefined;
    this.contractDExAddrs = [this.config.contractDExAddrs[0].addr];
    if (allContracts) {
      this.contractDExAddrs = this.config.contractDExAddrs.map(x => x.addr);
    }
      ...
      [
        (callbackSeries) => {
          this.utility.loadContract(
            ...
            this.contractDExAddrs[0],
            (err, contract) => {
              this.contractDEx = contract;
...
API.logs = function logs(callback, lookbackIn) {
  this.utility.blockNumber(this.web3, (err, blockNumber) => {
    ...
    async.mapSeries(
      this.contractDExAddrs,
      (contractDExAddr, callbackMap) => {
        const blocks = Object.values(this.eventsCache)
          .filter(x => x.address === contractDExAddr)
          .map(x => x.blockNumber);
...
API.getDExBalance = function getDExBalance(addr, callback) {
  if (addr.length === 42) {
...
API.getDExTokenBalances = function getDExTokenBalances(addr, callback) {
  if (addr.length === 42) {
    async.reduce(
      this.config.tokens,
      {},
...
API.getUSDBalance = function getUSDBalance(addr, tokenPrices, callback) {
  async.parallel(
    [
      ...
      (callbackParallel) => {
        API.getDExTokenBalances(addr, callbackParallel);
      },
      (callbackParallel) => {
    ...
    (err, results) => {
      const balances = { Wallet: results[0], DEx: results[1] };
...
API.addOrderFromEvent = function addOrderFromEvent(event, callback) {
  if (event.event === 'Order' && event.address === this.contractDExAddrs[0]) {
...
API.getTrades = function getTrades(callback) {
  ...
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractDExAddrs.indexOf(event.address) >= 0) {
...
API.getDepositsWithdrawals = function getDepositsWithdrawals(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Deposit' && this.contractDExAddrs.indexOf(event.address >= 0)) {
      ...
      event.event === 'Withdraw' && this.contractDExAddrs.indexOf(event.address) >= 0
    ) {
      if (event.args.amount.toNumber() > 0) {
        const token = API.getToken(event.args.token);
        depositsWithdrawals.push({
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        this.utility.sign(this.web3, addr, hash, pk, (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.contractDExAddrs[0],
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
