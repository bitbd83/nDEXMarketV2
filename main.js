...
DEx.prototype.ejs = function ejs(url, element, data) {
  if ($(`#${element}`).length) {
...
DEx.prototype.alertInfo = function alertInfo(message) {
  console.log(message);
...
DEx.prototype.alertDialog = function alertDialog(message) {
  console.log(message);
...
DEx.prototype.alertWarning = function alertWarning(message) {
  console.log(message);
...
DEx.prototype.alertError = function alertError(message) {
  console.log(message);
...
DEx.prototype.alertSuccess = function alertSuccess(message) {
  console.log(message);
...
DEx.prototype.txError = function txError(err) {
  ...
  utility.getBalance(this.web3, this.addrs[this.selectedAccount], (errBalance, resultBalance) => {
    ...
    if (this.connection.connection === 'RPC') {
      if (balance < 0.005) {
        this.alertError(
          `You tried to send an Ethereum transaction but there was an error. Your wallet's ETH balance (${balance} ETH) is not enough to cover the gas cost (Ethereum network fee). DEx sends 0.005 ETH with each transaction. This is an overestimate and the excess will get refunded to you. It's a good idea to send more than 0.005 so you can pay for not only this transaction, but also future transactions you do on DEx. The gas has to come directly from your Wallet (DEx has no physical way of paying gas from your deposited ETH).`);
        ga('send', {
          hitType: 'event',
          eventCategory: 'Error',
...
DEx.prototype.alertTxResult = function alertTxResult(err, txsIn) {
...
DEx.prototype.enableTooltipsAndPopovers = function enableTooltipsAndPopovers() {
  $('[data-toggle="popover"]').popover({ trigger: 'hover' });
  $('[data-toggle="tooltip"]').tooltip();
};
DEx.prototype.logout = function logout() {
...
DEx.prototype.createAccount = function createAccount() {
...
DEx.prototype.deleteAccount = function deleteAccount() {
...
DEx.prototype.selectAccount = function selectAccount(i) {
...
DEx.prototype.addAccount = function addAccount(newAddr, newPk) {
...
DEx.prototype.showPrivateKey = function showPrivateKey() {
...
DEx.prototype.addressLink = function addressLink(address) {
  return `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${address}`;
};
DEx.prototype.contractAddr = function contractAddr(addr) {
  this.config.contractDExAddr = addr;
  this.displayConnectionDescription(() => {});
  this.loading(() => {});
  this.refresh(() => {}, true, true);
};
DEx.prototype.displayAccounts = function displayAccounts(callback) {
...
DEx.prototype.displayLanguages = function displayLanguages(callback) {
...
DEx.prototype.selectLanguage = function selectLanguage(newLanguage) {
...
DEx.prototype.loadEvents = function loadEvents(callback) {
  let lastBlock = 0;
  Object.keys(this.eventsCache).forEach((id) => {
    const event = this.eventsCache[id];
    if (event.blockNumber > lastBlock && event.address === this.config.contractDExAddr) {
...
DEx.prototype.displayMyTransactions =
function displayMyTransactions(ordersIn, blockNumber, callback) {
  ...
  orders = orders.filter(order => order.order.contractAddr === this.config.contractDExAddr);
  ...
  events.forEach((event) => {
    try {
      if (
        event.event === 'Trade' &&
        event.address === this.config.contractDExAddr &&
...
DEx.prototype.displayVolumes = function displayVolumes(
  ...
  Object.keys(pairVolumes).forEach((pair) => {
    ...
    ordersFiltered = ordersFiltered.filter(
      order => order.order.contractAddr === this.config.contractDExAddr);
    // final order filtering and sorting
...
DEx.prototype.displayTradesAndChart = function displayTradesAndChart(callback) {
  // get the trade list
  const events = Object.values(this.eventsCache);
  const trades = [];
  events.forEach((event) => {
    if (event.event === 'Trade' && event.address === this.config.contractDExAddr) {
      if (event.args.amountGive.toNumber() > 0 && event.args.amountGet.toNumber() > 0) {
        // don't show trades involving 0 amounts
        let trade;
        if (
...
DEx.prototype.candlestickChart =
...
DEx.prototype.getOrdersByPair = function getOrdersByPair(tokenA, tokenB, callback) {
...
DEx.prototype.getReturnTicker = function getTopOrders(callback) {
...
DEx.prototype.getTopOrders = function getTopOrders(callback) {
...
DEx.prototype.displayOrderbook = function displayOrderbook(ordersIn, blockNumber, callback) {
  // only look at orders for the selected token and base
  let orders = ordersIn.filter(
    x =>
      (x.order.tokenGet.toLowerCase() === this.selectedToken.addr.toLowerCase() &&
...
DEx.prototype.displayTokensAndBases = function displayTokensAndBases(callback) {
...
DEx.prototype.displayAllBalances = function displayAllBalances(callback) {
...
DEx.prototype.transfer = function transfer(addr, inputAmount, toAddr) {
...
DEx.prototype.deposit = function deposit(addr, inputAmount) {
  let amount = new BigNumber(Number(utility.ethToWei(inputAmount, this.getDivisor(addr))));
  const token = this.getToken(addr);
  if (amount.lte(0)) {
      ...
      (err, result) => {
        ...
        if (amount.lte(result)) {
          utility.send(
            ...
            addr,
            'approve',
            [this.config.contractDExAddr, amount, { gas: this.config.gasApprove, value: 0 }],
            this.addrs[this.selectedAccount],
            this.pks[this.selectedAccount],
            this.nonce,
            (errSend, resultSend) => {
...
DEx.prototype.withdraw = function withdraw(addr, amountIn) {
...
DEx.prototype.order = function order(direction, amount, price, expires, refresh) {
...
DEx.prototype.publishOrder = function publishOrder(
...
DEx.prototype.cancelOrder = function cancelOrder(orderIn) {
...
DEx.prototype.addPending = function addPending(err, txsIn) {
  const txs = Array.isArray(txsIn) ? txsIn : [txsIn];
  txs.forEach((tx) => {
...
DEx.prototype.updateUrl = function updateUrl() {
...
DEx.prototype.getDivisor = function getDivisor(tokenOrAddress) {
...
DEx.prototype.getToken = function getToken(addrOrToken, name, decimals) {
  let result;
...
DEx.prototype.loadToken = function loadToken(addr, callback) {
...
DEx.prototype.selectToken = function selectToken(addrOrToken, name, decimals) {
...
DEx.prototype.selectBase = function selectBase(addrOrToken, name, decimals) {
...
DEx.prototype.selectTokenAndBase = function selectTokenAndBase(tokenAddr, baseAddr) {
...
DEx.prototype.displayBuySell = function displayBuySell(callback) {
...
DEx.prototype.displayTokenGuidesDropdown = function displayTokenGuidesDropdown() {
  const tokens = this.config.tokens.map(x => x);
  tokens.sort((a, b) => (a.name > b.name ? 1 : -1));
...
DEx.prototype.displayHelpDropdown = function displayHelpDropdown() {
  this.ejs(`${this.config.homeURL}/templates/helpDropdown.ejs`, 'helpDropdown', {});
};
DEx.prototype.displayHelp = function displayHelp(name) {
  $('#helpBody').html('');
...
DEx.prototype.displayScreencast = function displayScreencast(name) {
  $('#screencastBody').html('');
...
DEx.prototype.displayConnectionDescription = function displayConnectionDescription() {
  this.ejs(`${this.config.homeURL}/templates/connectionDescription.ejs`, 'connection', {
    connection: this.connection,
    contracts: this.config.contractDExAddrs,
    contractAddr: this.config.contractDExAddr,
    contractLink: `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${this.config.contractDExAddr}`,
  });
};
DEx.prototype.displayTokenGuide = function displayTokenGuide(name) {
...
DEx.prototype.checkContractUpgrade = function checkContractUpgrade() {
  if (
    (!this.selectedContract || this.selectedContract !== this.config.contractDExAddr) &&
    (this.addrs.length > 1 ||
      (this.addrs.length === 1 && this.addrs[0].slice(0, 39) !== '0x0000000000000000000000000000000000000'))
  ) {
    this.alertDialog(
      '<p>DEx has a new smart contract. It is now selected.</p><p>Please use the "Smart Contract" menu to select the old one and withdraw from it.</p><p><a href="javascript:;" class="btn btn-default" onclick="alertify.closeAll(); bundle.DEx.displayHelp(\'smartContract\')">Smart contract changelog</a></p>');
  }
};
DEx.prototype.resetCaches = function resetCaches() {
...
DEx.prototype.loading = function loading(callback) {
  [
    'deposit',
    'withdraw',
    'buy',
...
DEx.prototype.refresh = function refresh(callback, forceEventRead, initMarket, token, base) {
  this.q.push((done) => {
    if (token && base) {
    ...
    this.selectedContract = this.config.contractDExAddr;
...
DEx.prototype.refreshLoop = function refreshLoop() {
  const self = this;
  function loop() {
    self.refresh(() => {
...
DEx.prototype.initDisplays = function initDisplays(callback) {
  this.loading(() => {});
...
DEx.prototype.loadWeb3 = function loadWeb3(callback) {
  this.config = config;
  // web3
...
DEx.prototype.initContracts = function initContracts(callback) {
  this.web3.version.getNetwork((error, version) => {
    ...
    // connection
    this.config.contractDExAddr = this.config.contractDExAddrs[0].addr;
    // get accounts
    ...
    utility.loadContract(
      this.web3,
      this.config.contractDEx,
      this.config.contractDExAddr,
      (err, contractDEx) => {
        this.contractDEx = contractDEx;
        utility.loadContract(
          this.web3,
...
DEx.prototype.startDEx = function startDEx() {
...
module.exports = { DEx: dEx, utility };
